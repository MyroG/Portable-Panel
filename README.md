...
| Constraint Mode | Defines how the panel behaves relative to the player:<br>• **None** – The panel stays where it got placed in world space.<br>• **Position** – The panel sits in the player's playspace and moves with them.<br>• **View** – The panel stays in front of the player's face like a HUD.<br>Note : If you're using the throwable panel, then setting this parameter to anything except `none` won't make the panel throwable anymore. |
